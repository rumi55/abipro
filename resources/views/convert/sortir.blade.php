...
<div class="card elevation-2">
    ...
    <h3>{{__('Sortir Conversion')}}</h3>
    </div>
    <form action="{{route('convert.upload', 'gls')}}" method="POST" enctype="multipart/form-data" >
      @csrf
      <div class="card-body">
        <div class="form-group">
        <label>Tags Group Name</label>
        <input  required type="text" class="form-control" name="name" id="name" placeholder="" value="">
        <small class="text-muted"></small>
        </div>
        <div class="form-group">
        <label>Browse gls.dbf file</label>
