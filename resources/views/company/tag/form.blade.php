    ...
    @component('components.card_form', [
      ...
      @endslot
        <div class="form-group row">
            <label for="group" class="col-sm-2 col-form-label">Nama Sortir</label>
            <div class="col-md-6 col-sm-12">
            <input type="text" autocomplete="off" data-url="{{route('tags.groups')}}" required class="form-control autocomplete @error('group') is-invalid @enderror" name="group" id="group" value="{{old('group', $model->group)}}">
            @error('group') <small class="text-danger">{!! $message !!}</small> @enderror
            </div>
        </div>
        <div class="form-group row">
            <label for="item_id" class="col-sm-2 col-form-label">Kode Item</label>
            <div class="col-md-6 col-sm-10">
            <input type="text" required class="form-control @error('item_id') is-invalid @enderror  @error('display_name') is-invalid @enderror" name="item_id" id="item_id" value="{{old('item_id', $model->item_id)}}">
