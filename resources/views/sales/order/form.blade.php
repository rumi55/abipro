...
@section('content')
<form method="POST" id="transaction-form" action="{{$mode=='edit'?route('sales_orders.edit.update', ['type'=>$transaction->trans_type, 'id'=>$transaction->id]):route('sales_orders.create.save')}}">
@csrf
@if($mode=='edit')
@method('PUT')
@endif
<div class="card">
    <div class="card-header">
        <h5 class="card-title">{{$mode=='create'?trans('New Sales Order'):trans('Sales Order').' #'.$transaction->trans_no}}</h5>
    </div>
    <div class="card-body pb-1">  
        <div class="row">    
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <label for="trans_no" >{{__('Transaction Number')}}</label>
            ...
            @if(!empty($transaction->sales_quote_id))
            <div class="col-md-3">
                <div class="form-group">
                    <label for="trans_no" >{{__('Quote No.')}}</label><br>
                    <div class="mt-2">
                    <a href="{{route('sales_quotes.view', $transaction->sales_quote_id)}}">{{$transaction->salesQuote->trans_no}}</a>                    
                    <input id="sales_quote_id" name="transaction[sales_quote_id]" type="hidden" class="form-control" value="{{$transaction->sales_quote_id}}" >
                    </div>
                </div>
            </div> 
            @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2 col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input style="min-width:80px" name="transaction[detail][{{$i}}][discount_percent]" data-index="{{$i}}" type="text" id="detail_discount_percent_{{$i}}" class="form-control @error('transaction.detail.'.$i.'.discount_percent') is-invalid @enderror number discount_percent calc" value="{{old('transaction.detail.'.$i.'.discount_percent', empty($detail->discount_percent)?'0,00':$detail->discount_percent)}}"  data-inputmask="'alias':'decimal', 'groupSeparator': '.', 'radixPoint':',', 'autoGroup': true, 'digits': 2, 'digitsOptional': false, 'prefix': ''" data-mask>
