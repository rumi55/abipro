...
<div class="card">
    <div class="card-header">
        <h5 class="card-title">{{__('Sales Invoice')}} #{{$transaction->trans_no}}</h5>
    ...
    <div class="card-footer">
        <div class="row">
            <div class="col-sm-6">
            @if(!empty($prev_id))
            <a title="{{__('Previouse')}}" href="{{route('sales_invoices.view', $prev_id)}}"><i class="fas fa-chevron-left"></i></a>
            @endif
            </div>
            <div class="col-sm-6 text-right">
            @if(!empty($next_id))
            <a title="{{__('Next')}}" href="{{route('sales_invoices.view', $next_id)}}"><i class="fas fa-chevron-right"></i></a>
            @endif
            </div>
        </div>
    </div>
