@php 
$active_menu="sales_invoices";
$page_title = trans('Sales Invoice');
$breadcrumbs = array(
    ['label'=>trans('Sales'), 'url'=>route('dcru.index','sales_invoices')],
    ['label'=>trans('Sales Invoice'), 'url'=>route('dcru.index','sales_invoices')],
...
@section('content')
<form method="POST" id="transaction-form" action="{{$mode=='edit'?route('sales_invoices.edit.update', ['type'=>$transaction->trans_type, 'id'=>$transaction->id]):route('sales_invoices.create.save')}}">
@csrf
@if($mode=='edit')
@method('PUT')
@endif
<div class="card">
    <div class="card-header">
        <h5 class="card-title">{{$mode=='create'?trans('New Sales Invoice'):trans('Sales Invoice').' #'.$transaction->trans_no}}</h5>
    </div>
