...
class SalesOrderController extends Controller
{
    public function create(Request $request){
        ...
        $mode = 'create';
        return view('sales.order.form', compact('transaction', 'mode'));
    }
    ...
    public function duplicate(Request $request, $id){
        $company_id = company('id');
        $transaction = SalesOrder::findOrFail($id);
        $mode = 'create';
    ...
    public function view($id){
        $company_id = company('id');
        $transaction = SalesOrder::findOrFail($id);
        $prev=SalesOrder::where('company_id', $transaction->company_id)
        ->where('id','<', $transaction->id)->orderBy('id', 'desc')->first();
        $next=SalesOrder::where('company_id', $transaction->company_id)
        ->where('id','>', $transaction->id)->orderBy('id', 'asc')->first();
        $prev_id = $prev!=null?encode($prev->id):'';
        $next_id = $next!=null?encode($next->id):'';
        return view('sales.order.view', compact('transaction', 'next_id', 'prev_id'));
    }
    ...
    public function save(Request $request){
        ...
        if(empty($transaction['numbering_id'])){
            $rules['trans_no'] = 'required|max:16|unique:sales_orders,trans_no,NULL,id,company_id,'.$company_id;                
        }else{
        ...
        try{
            ...
            if(!empty($transaction['numbering_id'])){
                ...
                do{
                    ...
                    $exists = SalesOrder::where('trans_no', $trans_no)->where('company_id', $company_id)->exists(); 
                    if(!$exists){
                        $counter->save();
            ...
            $sales = SalesOrder::create($transaction);
            foreach($transaction['detail'] as $i => $detail){
                $detail['sales_order_id'] = $sales->id;
                $detail['sequence'] = $i;
        ...
        add_log('sales_orders', 'create', '');
        return redirect()->route('sales_orders.view', $sales->id)->with('success', trans('New :attr has been created.', ['attr'=>strtolower(trans('Sales order'))]));
    }
    public function update(Request $request, $id){
        ...
        try{
            ...
            foreach($transaction['detail'] as $i => $detail){
                ...
                SalesOrderDetail::updateOrCreate(
                    ['sequence'=>$i, 'sales_order_id'=>$sales->id],
                $detail);
            }
            \DB::commit();        
        }catch(Exception $e){
        ...
        add_log('sales_orders', 'update', '');
        return redirect()->route('sales_orders.view', $sales->id)->with('success', trans('Changes have been saved.'));
    }
    
    public function delete($id){
        $transaction = SalesOrder::findOrFail($id);
        $transaction->delete();
        add_log('sales_orders', 'delete', '');
        return redirect()->route('dcru.index', 'sales_orders')->with('success', trans(':attr deleted.', ['attr'=>strtolower(trans('Sales order'))]));
    }
