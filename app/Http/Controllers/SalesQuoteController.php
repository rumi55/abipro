...
class SalesQuoteController extends Controller
{
    public function create(Request $request){
        ...
        $transaction = new SalesQuote;
        $mode = 'create';
        return view('sales.quote.form', compact('transaction', 'mode'));
    }
    public function duplicate(Request $request, $id){
        $company_id = company('id');
        $transaction = SalesQuote::findOrFail($id);
        $mode = 'create';
    ...
    public function view($id){
        $company_id = company('id');
        $transaction = SalesQuote::findOrFail($id);
        $prev=SalesQuote::where('company_id', $transaction->company_id)
        ->where('id','<', $transaction->id)->orderBy('id', 'desc')->first();
        $next=SalesQuote::where('company_id', $transaction->company_id)
        ->where('id','>', $transaction->id)->orderBy('id', 'asc')->first();
        $prev_id = $prev!=null?encode($prev->id):'';
        $next_id = $next!=null?encode($next->id):'';
        return view('sales.quote.view', compact('transaction', 'next_id', 'prev_id'));
    }
    ...
    public function save(Request $request){
        ...
        if(empty($transaction['numbering_id'])){
            $rules['trans_no'] = 'required|max:16|unique:sales_quotes,trans_no,NULL,id,company_id,'.$company_id;                
        }else{
        ...
        try{
            ...
            if(!empty($transaction['numbering_id'])){
                ...
                do{
                    ...
                    $exists = SalesQuote::where('trans_no', $trans_no)->where('company_id', $company_id)->exists(); 
                    if(!$exists){
                        $counter->save();
            ...
            $sales = SalesQuote::create($transaction);
            foreach($transaction['detail'] as $i => $detail){
                $detail['sales_quote_id'] = $sales->id;
                $detail['sequence'] = $i;
                ...
                SalesQuoteDetail::create($detail);
            }
            \DB::commit();        
        }catch(Exception $e){
            \DB::rollback();
        }
        add_log('sales_quotes', 'create', '');
        return redirect()->route('sales_quotes.view', $sales->id)->with('success', trans('New :attr has been created.', ['attr'=>strtolower(trans('Sales Quote'))]));
    }
    public function update(Request $request, $id){
        $sales = SalesQuote::findOrFail($id); 
        $transaction =$request->transaction;
        $company_id = company('id');
        $rules = [
            'transaction.customer_id'=>'required|integer',
        ...
        try{
            ...
            foreach($transaction['detail'] as $i => $detail){
                ...
                $detail['discount_percent'] = parse_number($detail['discount_percent']);
                $detail['tax'] = parse_number($detail['tax']);
                SalesQuoteDetail::updateOrCreate(
                    ['sequence'=>$i, 'sales_quote_id'=>$sales->id],
                $detail);
            }
            \DB::commit();        
        }catch(Exception $e){
        ...
        add_log('sales_quotes', 'update', '');
        return redirect()->route('sales_quotes.view', $sales->id)->with('success', trans('Changes have been saved.'));
    }
    
    public function delete($id){
        $transaction = SalesQuote::findOrFail($id);
        $transaction->delete();
        add_log('sales_quotes', 'delete', '');
        return redirect()->route('dcru.index', 'sales_quotes')->with('success', trans(':attr deleted.', ['attr'=>strtolower(trans('Sales Quote'))]));
    }
