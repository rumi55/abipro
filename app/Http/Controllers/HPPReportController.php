...
class HPPReportController extends Controller
{
    public function index(Request $request){
        ...
        $title = trans('Harga Pokok Produksi');
        $view = 'report.hpp.default';
    ...
    private function query($params, $company){
        ...
        // DB::enableQueryLog();
        $mappings = \App\AccountMapping::where('id','<=', 7)->orderBy('id')->get();
        $data = array();

        foreach ($mappings as $mapping) {
            $mapping_id = $mapping->id;
            $query = DB::table(DB::raw('vw_accounts a'))
            ->select($select)
            ->whereRaw("a.company_id=$company->id")
            ->where('a.account_mapping', $mapping_id)
            ->distinct()
    ...
    private function query_profit($params, $company){
        $select = [
            DB::raw('a.id, a.sequence, a.account_no, a.account_type, a.account_type_id, a.account_group, a.account_name, a.tree_level')
        ];
        $cutoff_date = $params['cutoff_date'];
